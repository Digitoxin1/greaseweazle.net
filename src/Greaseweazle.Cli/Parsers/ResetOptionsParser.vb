...
Namespace Greaseweazle.Cli.Parsers
    ...
    ' Argv parser for `gw-vb reset`. The library's ResetOptions DTO is now
    ' a pure POCO; the CLI calls Parse(args) to build it. Migrated verbatim
    ' from the legacy Reset.BuildRuntimePreview so the user-visible argv
    ' parsing semantics (token splitting, --flag=value handling, error
    ' messages mentioning --device, etc.) are unchanged.
    Public NotInheritable Class ResetOptionsParser

        Private Const ActionName As String = "reset"
        ...
        Public Shared Function Parse(args As IReadOnlyList(Of String)) As ResetOptions
