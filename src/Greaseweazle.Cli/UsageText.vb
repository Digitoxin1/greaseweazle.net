...
Namespace Greaseweazle.Cli
    ...
    Public Module UsageText
        ...
        Private ReadOnly ActionHelpLines As New Dictionary(Of String, String())(StringComparer.OrdinalIgnoreCase) From {
            ...
            {"delays", New String() {"usage: gw-vb delays [options]", "", "options:", "  -h, --help            show this help message and exit", "  --device <COMx>       device name (COM/serial port)", "  --select <us>         select delay", "  --step <us>           step delay", "  --settle <ms>         settle delay", "  --motor <ms>          motor delay", "  --watchdog <ms>       watchdog timeout", "  --pre-write <us>      pre-write delay", "  --post-write <us>     post-write delay", "  --index-mask <us>     index mask delay", "  --test                dry-run preview only (skip hardware access)", "", "Select Delay: (usec) Delay after asserting drive select", "Step Delay:   (usec) Delay after issuing a head-step command", "Settle Time:  (msec) Delay after completing a head-seek operation", "Motor Delay:  (msec) Delay after turning on drive spindle motor", "Watchdog:     (msec) Timeout since last command upon which all drives", "                     are deselected and spindle motors turned off", "Pre-Write:    (usec) Min. time from track change to write start", "Post-Write:   (usec) Min. time from write end to track change", "Index Mask:   (usec) Index post-trigger mask time"}},
